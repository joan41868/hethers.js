...
const logger = new hethers.utils.Logger(version);
const Guard = {};
function hexConcat(values) {
    return hethers.utils.hexlify(hethers.utils.concat(values.map((v) => {
        if (v instanceof Opcode) {
            return [v.value];
        }
        if (typeof (v) === "number") {
            if (v >= 0 && v <= 255 && !(v % 1)) {
                return hethers.utils.hexlify(v);
            }
            else {
...
class Script {
    constructor(filename, callback) {
        hethers.utils.defineReadOnly(this, "filename", filename);
        hethers.utils.defineReadOnly(this, "contextObject", this._baseContext(callback));
        hethers.utils.defineReadOnly(this, "context", vm.createContext(this.contextObject));
    }
    _baseContext(callback) {
        return new Proxy({
            ...
            hethers: hethers,
            utils: hethers.utils,
            BigNumber: hethers.BigNumber,
            arrayify: hethers.utils.arrayify,
            concat: hexConcat,
            hexlify: hethers.utils.hexlify,
            zeroPad: function (value, length) {
                return hethers.utils.hexlify(hethers.utils.zeroPad(value, length));
            },
            id: hethers.utils.id,
            ...
            randomBytes: function (length) {
                return hethers.utils.hexlify(hethers.utils.randomBytes(length));
            },
            toUtf8Bytes: hethers.utils.toUtf8Bytes,
            toUtf8String: hethers.utils.toUtf8String,
            formatBytes32String: hethers.utils.formatBytes32String,
            parseBytes32String: hethers.utils.parseBytes32String,
            Opcode: Opcode,
            sighash: function (signature) {
                return hethers.utils.id(hethers.utils.FunctionFragment.from(signature).format()).substring(0, 10);
            },
            topichash: function (signature) {
                return hethers.utils.id(hethers.utils.EventFragment.from(signature).format());
            },
            assemble: assemble,
...
export class Node {
    constructor(guard, location, options) {
        ...
        hethers.utils.defineReadOnly(this, "location", Object.freeze(location));
        hethers.utils.defineReadOnly(this, "tag", `node-${nextTag++}-${this.constructor.name}`);
        for (const key in options) {
            hethers.utils.defineReadOnly(this, key, options[key]);
        }
    }
...
export class ValueNode extends Node {
    ...
    getPushLiteral(value) {
        ...
        const hex = hethers.utils.hexlify(value);
        if (hex === "0x") {
            throwError("invalid literal: 0x", this.location);
        }
        // Make sure it will fit into a push
        const length = hethers.utils.hexDataLength(hex);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.verbatim) {
                if (this.value.substring(0, 2) === "0x") {
                    ...
                    visit(this, hethers.BigNumber.from(this.value).toHexString());
                }
            }
            else {
                visit(this, this.getPushLiteral(hethers.BigNumber.from(this.value)));
            }
            assembler.end(this);
        });
    }
            ...
            if (isOffset && assembler.positionIndependentCode) {
                ...
                if (here > value) {
                    ...
                    for (let w = 1; w <= 5; w++) {
                        ...
                        if (hethers.utils.hexDataLength(literal) <= w) {
                            literal = hethers.utils.hexZeroPad(literal, w);
                            break;
                        }
                    }
...
export class OpcodeNode extends ValueNode {
    ...
    assemble(assembler, visit) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // Append this opcode
            visit(this, hethers.utils.hexlify(this.opcode.value));
            assembler.end(this);
        });
    }
    children() {
...
export class LabelledNode extends Node {
    constructor(guard, location, name, values) {
        ...
        values = hethers.utils.shallowCopy(values || {});
        values.name = name;
...
export class LabelNode extends LabelledNode {
    assemble(assembler, visit) {
        return __awaiter(this, void 0, void 0, function* () {
            assembler.start(this);
            visit(this, hethers.utils.hexlify(Opcode.from("JUMPDEST").value));
            assembler.end(this);
        });
    }
    static from(options) {
...
export class PaddingNode extends ValueNode {
    ...
    assemble(assembler, visit) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            padding.fill(0);
            visit(this, hethers.utils.hexlify(padding));
            assembler.end(this);
        });
    }
}
export class DataNode extends LabelledNode {
    constructor(guard, location, name, data) {
        ...
        hethers.utils.defineReadOnly(this, "padding", new PaddingNode(Guard, this.location));
    }
    assemble(assembler, visit) {
        return __awaiter(this, void 0, void 0, function* () {
            assembler.start(this);
            ...
            const bytecode = hethers.utils.concat(this.data.map((d) => assembler.getBytecode(d)));
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.verbatim) {
                if (typeof (result) === "number") {
                    visit(this, hethers.BigNumber.from(result).toHexString());
                }
                else {
                    visit(this, hethers.utils.hexlify(result));
                }
            }
            else {
...
export function disassemble(bytecode) {
    const ops = [];
    const offsets = {};
    const bytes = hethers.utils.arrayify(bytecode, { allowMissingPrefix: true });
    let i = 0;
    let oob = false;
    while (i < bytes.length) {
        let opcode = Opcode.from(bytes[i]);
        if (!opcode) {
            opcode = new Opcode(`unknown (${hethers.utils.hexlify(bytes[i])})`, bytes[i], 0, 0);
        }
        ...
        if (push) {
            const data = hethers.utils.hexlify(bytes.slice(i, i + push));
            if (hethers.utils.hexDataLength(data) === push) {
                op.pushValue = data;
                op.length += push;
                i += push;
            }
    ...
    ops.getBytes = function (offset, length) {
        ...
        return hethers.utils.arrayify(result);
    };
...
export function formatBytecode(bytecode) {
    ...
    bytecode.forEach((op) => {
        ...
        let offset = hethers.utils.hexZeroPad(hethers.utils.hexlify(op.offset), 2);
...
class Assembler {
    constructor(root, positionIndependentCode) {
        hethers.utils.defineReadOnly(this, "root", root);
        hethers.utils.defineReadOnly(this, "positionIndependentCode", !!positionIndependentCode);
        const nodes = {};
        const labels = {};
        const parents = {};
        ...
        root.visit((node) => {
            ...
            if (node instanceof LabelledNode) {
                ...
                if (labels[node.name]) {
                    logger.throwError(("duplicate label: " + node.name), hethers.utils.Logger.errors.UNSUPPORTED_OPERATION, {});
                }
        ...
        root.visit((node) => {
            ...
            if (node instanceof LinkNode) {
                ...
                if (!target) {
                    logger.throwError(("missing label: " + node.label), hethers.utils.Logger.errors.UNSUPPORTED_OPERATION, {});
                }
            }
        ...
        hethers.utils.defineReadOnly(this, "labels", Object.freeze(labels));
        hethers.utils.defineReadOnly(this, "nodes", Object.freeze(nodes));
        hethers.utils.defineReadOnly(this, "_parents", Object.freeze(parents));
    }
    // Link operations
    getTarget(label) {
    ...
    getLinkValue(target, source) {
        ...
        const bytes = Array.prototype.slice.call(hethers.utils.arrayify(info.bytecode));
        hethers.utils.defineReadOnly(bytes, "ast", target);
        hethers.utils.defineReadOnly(bytes, "source", target.location.source);
        ...
        if (!safeOffset) {
            ...
            hethers.utils.defineReadOnly(bytes, "_freeze", function () { });
        }
        ...
        if (safeOffset) {
            ...
            hethers.utils.defineReadOnly(bytes, "_freeze", function () {
                ...
                frozen = true;
                hethers.utils.defineReadOnly(bytes, "offset", bytes.offset);
            });
        }
        return bytes;
    }
...
class CodeGenerationAssembler extends Assembler {
    constructor(root, options) {
        ...
        hethers.utils.defineReadOnly(this, "retry", ((options.retry != null) ? options.retry : 512));
        hethers.utils.defineReadOnly(this, "filename", resolve(options.filename || "./contract.asm"));
        hethers.utils.defineReadOnly(this, "defines", Object.freeze(options.defines || {}));
        hethers.utils.defineReadOnly(this, "_stack", []);
        this.reset();
    }
    _didChange() {
        this._changed = true;
        ...
        try {
            if (result.offset < 0) {
                ...
                this._checks.push(() => {
                    ...
                    if (check.offset === result.offset && hethers.utils.hexlify(check) === hethers.utils.hexlify(result)) {
                        return true;
                    }
                    return false;
                });
            ...
            this._checks.push(() => {
                const check = super.getLinkValue(target, source);
                return (hethers.utils.hexlify(check) === hethers.utils.hexlify(result));
            });
        }
        return result;
    }
    ...
    _assemble() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.root.assemble(this, (node, bytecode) => {
                ...
                offset += hethers.utils.hexDataLength(bytecode);
            });
            this._runChecks();
        });
    }
    assemble(label) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return logger.throwError(`unable to assemble; ${this.retry} attempts failed to generate stable bytecode`, hethers.utils.Logger.errors.UNKNOWN_ERROR, {});
        });
    }
}
