...
describe('Checksum and ICAP address generation', function () {
    ...
    tests.forEach(function (test) {
        it(('correctly transforms address - ' + test.name), function () {
            assert_1.default.equal(ethers_1.ethers.utils.getAddress(test.address.toLowerCase()), test.checksumAddress.toLowerCase(), 'correctly computes checksum address from address');
            ...
            assert_1.default.equal(ethers_1.ethers.utils.getAddress(test.checksumAddress.toLowerCase()), test.checksumAddress.toLowerCase(), 'correctly computes checksum address from checksum address');
            assert_1.default.equal(ethers_1.ethers.utils.getIcapAddress(test.checksumAddress.toLowerCase()), test.icapAddress, 'correctly computes ICAP address from checksum address');
