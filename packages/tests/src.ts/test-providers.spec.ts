...
import { HederaTransactionResponse, TransactionResponse, TransactionReceipt } from "@ethersproject/abstract-provider";
...
describe("Test Hedera Provider", function () {
    ...
    it.only("Should populate txn response", async function (){
    ...
    /* This test is skipped because the previewnet will be resetted */
    it.only("Should return null on record not found", async function (){
    ...
    /* This test is skipped because the previewnet will be resetted */
    it.only("Should throw backend error", async function (){
...
describe("Test Hedera Provider Formatters", function () {
    ...
    it('Should parse hedera response to hethers response', async function () {
        const hederaTransactionResponse: HederaTransactionResponse = {
            amount: 0,
        ...
        assert.strictEqual(transactionResponse.to, hederaTransactionResponse.to);
        assert.strictEqual(transactionResponse.from, hederaTransactionResponse.from);
        assert.strictEqual(transactionResponse.hash, hederaTransactionResponse.hash);
        assert.strictEqual(transactionResponse.transactionId, hederaTransactionResponse.transaction.transaction_id);
        assert.strictEqual(transactionResponse.gasLimit.toNumber(), hederaTransactionResponse.gas_limit);

        assert.strictEqual(transactionResponse.customData.gas_used, hederaTransactionResponse.gas_used);
        assert.strictEqual(transactionResponse.customData.call_result, hederaTransactionResponse.call_result);
        assert.deepStrictEqual(transactionResponse.customData.logs, hederaTransactionResponse.logs);
        assert.deepStrictEqual(transactionResponse.customData.transaction, hederaTransactionResponse.transaction);
    }).timeout(timeout);
